...
class Cart extends Component{
    ...
    render(){
        ...
        return <div className='boxcart'>
           <h5>购物车</h5>
           <div className='cartcont'>
               <div style={{background:'#fff',display:'flex',height:'20px',lineHeight:'20px',borderBottom: "1px solid #ccc",position:'sticky',top:'0',left:'0'}}>
                    
                    <input type='checkbox' 
                    onChange={()=>{this.props.allcheck()}} 
                    checked={this.props.allchecked} style={{margin:'4px'}}/>
                    <p>多点超市</p>
               </div>
                {
                    this.props.cartlist.map((item,index)=>{
                        return <dl key={index}>
                            <input type='checkbox'
                            onChange={()=>{this.props.checkbtn(item)}} 
                            checked={item.flage}  style={{margin:'30px 10px'}}/>
                            <dt>
